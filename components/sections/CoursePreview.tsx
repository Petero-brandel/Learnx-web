...
import { ArrowRight, ArrowLeft, BookOpen, Layers, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function CoursePreview() {
 ...
 return (
 <section id="courses" className="py-28 px-4 sm:px-6 lg:px-8 bg-white">
  ...
  <Reveal>
   ...
   <div className="flex items-center gap-3">
    ...
    className="h-10 w-10 rounded-full border border-zinc-200 bg-white flex items-center justify-center text-zinc-500 hover:text-zinc-900 hover:border-zinc-400 hover:shadow-sm transition-all duration-200 disabled:opacity-40"
  ...
  {loading && (
   ...
   {[1, 2, 3].map((i) => (
    <div key={i} className="min-w-[300px] flex-1 rounded-2xl border border-zinc-100 bg-white overflow-hidden animate-pulse">
  ...
  {!loading && courses.length === 0 && (
   <div className="text-center py-20 rounded-2xl border border-dashed border-zinc-200 bg-white/50">
   ...
   >
    ...
    return (
     ...
     <Link
      ...
      className="group flex flex-col h-full rounded-2xl border border-zinc-200 bg-white overflow-hidden hover:shadow-md hover:border-zinc-300 transition-all duration-300"
     >
      {/* Thumbnail */}
      <div className="relative h-48 w-full overflow-hidden flex items-center justify-center bg-zinc-100">
      {course.thumbnail ? (
       <Image
