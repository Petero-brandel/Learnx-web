...
export function AuthDialog({ variant, mode }: { variant: Variant; mode: Mode }) {
  ...
  const closeTo = useMemo(() => (mode === 'modal' ? () => router.back() : () => router.push('/')), [mode, router]);
  ...
  const shell = (
    <div className="min-h-screen bg-zinc-950/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 lg:p-8 font-sans text-zinc-950 dark:text-zinc-50">
      <div
      ...
      >
        <div className="text-center mb-6">
          <Link href="/" className="inline-flex items-center justify-center rounded-full px-3 py-1 text-2xl font-bold tracking-tight hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors" onClick={closeTo}>
            LearnX
          </Link>
        </div>
  ...
  if (mode === 'modal') {
    return (
      <div className="fixed inset-0 z-[100]" onClick={closeTo}>
        {shell}
      </div>
    );
  }
