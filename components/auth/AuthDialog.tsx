...
export function AuthDialog({ variant, mode }: AuthDialogProps) {
  ...
  if (isVerificationSent) {
    CardContent = (
      <div
        className="w-full max-w-[480px] relative z-10 rounded-[28px] border border-zinc-200/80 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-[0_30px_120px_rgba(0,0,0,0.24)] backdrop-blur-2xl p-6 sm:p-10 text-center"
    ...
    CardContent = (
      <div
        className="w-full max-w-[480px] relative z-10 rounded-[28px] border border-zinc-200/80 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-[0_30px_120px_rgba(0,0,0,0.24)] backdrop-blur-2xl p-6 sm:p-8"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          type="button"
        ...
        <form className="space-y-4" onSubmit={handleSubmit} noValidate>
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between gap-4">
              ...
              {!isSignup && (
                <a href="/forgot-password" className="text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:underline">
                  Forgot?
                </a>
              )}
            </div>
  ...
  if (mode === 'modal') {
    return (
      <div
        className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/40 backdrop-blur-sm p-4 sm:p-6 lg:p-8 animate-in fade-in duration-200"
        onClick={closeTo}
      >
        {CardContent}
      </div>
