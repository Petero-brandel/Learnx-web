...
function ResetPasswordContent() {
  ...
  if (!uid || !token) {
    return (
      <div className="w-full max-w-[480px] relative z-10 rounded-[28px] border border-zinc-200/80 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-[0_30px_120px_rgba(0,0,0,0.24)] backdrop-blur-2xl p-6 sm:p-10 text-center">
        ...
        <Link href="/forgot-password" className="flex w-full justify-center rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-indigo-700">
          Request new link
        </Link>
      </div>
    );
  ...
  if (status === 'success') {
    return (
      <div className="w-full max-w-[480px] relative z-10 rounded-[28px] border border-zinc-200/80 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-[0_30px_120px_rgba(0,0,0,0.24)] backdrop-blur-2xl p-6 sm:p-10 text-center">
        ...
        <Link href="/login" className="flex w-full justify-center rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-indigo-700">
          Log in
        </Link>
      </div>
    );
  ...
  return (
    <div className="w-full max-w-[480px] relative z-10 rounded-[28px] border border-zinc-200/80 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-[0_30px_120px_rgba(0,0,0,0.24)] backdrop-blur-2xl p-6 sm:p-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className={`w-full rounded-2xl border bg-white dark:bg-zinc-950 px-3.5 py-3 text-sm outline-none transition-all placeholder:text-zinc-400 ${fieldErrors.confirm ? 'border-red-300 focus:ring-red-500' : 'border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-indigo-500 focus:border-transparent'}`}
          />
...
export default function ResetPasswordPage() {
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#121212] relative flex items-center justify-center p-4 lg:p-8 font-sans text-zinc-950 dark:text-zinc-50 selection:bg-indigo-500/30 overflow-hidden">
      <Suspense fallback={
        <div className="w-full max-w-[480px] relative z-10 rounded-[28px] border border-zinc-200/80 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/95 shadow-[0_30px_120px_rgba(0,0,0,0.24)] backdrop-blur-2xl p-6 sm:p-10 text-center">
