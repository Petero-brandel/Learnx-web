...
export default function LoginModal() {
  ...
  return (
    <div 
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8 bg-zinc-900/10 dark:bg-black/20 backdrop-blur-sm animate-in fade-in duration-300"
      onClick={() => router.back()}
    >
      {/* Main Card */}
      <div 
        className="relative w-full max-w-[480px] bg-white/95 dark:bg-zinc-950/90 backdrop-blur-2xl border border-zinc-200 dark:border-zinc-800 shadow-2xl rounded-3xl p-6 sm:p-8 animate-in zoom-in-95 duration-300"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Logo */}
        <div className="text-center mb-6">
          <Link href="/" className="inline-block font-bold text-2xl tracking-tight" onClick={() => router.back()}>
            LearnX
          </Link>
        </div>
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <Link href="/signup" replace className="text-indigo-600 dark:text-indigo-400 font-medium hover:underline">
              Sign up
            </Link>
          </p>
        </div>
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link href="/forgot-password" onClick={() => router.back()} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-medium">
                Forgot?
              </Link>
            </div>
